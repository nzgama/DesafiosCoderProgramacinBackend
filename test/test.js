...
describe("test all endpoint", () => {
  ...
  describe("POST ONE", () => {
    it("deberia incorporar un posteo nuevo", async () => {
      ...
      const res = await request.post("/api/productos/add").send(post);
  ...
  //PUT
  describe("PUT ONE", () => {
    it("deberia incorporar un posteo nuevo", async (id) => {
      const post = generatePost();
  ...
  describe("PUT ONE", () => {
    it("deberia incorporar un posteo nuevo", async (id) => {
      const post = generatePost();
      const res = await request.delete("/api/productos/" + id).send(post);
      expect(res.status).to.eql(201);
      expect(res.body).to.be.a("object");
      expect(res.body).to.include.keys("title", "body", "id");
      expect(post.title).to.eql(res.body.title);
